...
Có ba cách để di chuyển dữ liệu từ cài đặt RisuAI hiện có (RisuAI Web, RisuAI Cục bộ) sang Kei-Risu. Chọn dựa trên môi trường nguồn và kích thước dữ liệu của bạn.
...
Xuất tệp sao lưu `.bin` từ RisuAI hiện có, sau đó nhập vào Kei-Risu. Hoạt động bất kể môi trường nguồn (web / Tauri / Capacitor / cục bộ).

1. **Trong RisuAI hiện có**: Cài đặt > Sao lưu > "Lưu bản sao lưu cục bộ" để xuất tệp `.bin`.
2. **Trong Kei-Risu**: Cài đặt > Di chuyển dữ liệu > "Nhập bản sao lưu cục bộ Risu gốc" để nhập tệp `.bin`.
...
2. Trong Kei-Risu, vào Cài đặt > Di chuyển dữ liệu > accordion "Nhập thư mục save từ NodeOnly Risu".
...
1. Dừng máy chủ Kei-Risu.
2. Ghi đè thư mục `save` của Kei-Risu bằng thư mục `save` của RisuAI hiện có.
3. Khởi động lại máy chủ Kei-Risu — quá trình di chuyển tự động bắt đầu.
