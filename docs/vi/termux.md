...
Hướng dẫn này giải thích cách xây dựng và chạy Kei-Risu trực tiếp trên điện thoại Android sử dụng Termux. Mẫu sử dụng dự kiến là mở `http://localhost:6001` trong trình duyệt của chính điện thoại.
...
Kei-Risu không cung cấp tệp nhị phân biên dịch sẵn cho Termux, vì vậy điện thoại tự xây dựng mọi thứ. Bao gồm các module gốc như `better-sqlite3`, quá trình xây dựng mất khoảng **10 đến 40 phút**, tùy theo hiệu năng điện thoại.
...
> Các bảo trì viên Termux đã ngừng cập nhật bản Play Store vào năm 2020, và bản này không còn cài đặt được các gói mới mà Kei-Risu cần (Node.js 22+).
...
1. Cài đặt `git`
2. Sao chép kho lưu trữ Kei-Risu
...
Giao diện Kei-Risu sẽ tải lên. `localhost` được trình duyệt tự động xem là ngữ cảnh bảo mật (secure context), vì vậy clipboard, `crypto.subtle` và các API yêu cầu ngữ cảnh bảo mật khác đều hoạt động bình thường.
...
Kei-Risu tự động phát hiện môi trường Termux và hiển thị cảnh báo trong menu Truy cập từ xa, ẩn nút khởi động.
