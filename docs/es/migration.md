...
Hay tres formas de migrar datos desde una instalación existente de RisuAI (RisuAI Web, RisuAI Local) a Kei-Risu. Elija según su entorno de origen y el tamaño de los datos.
...
Exporte un archivo de copia de seguridad `.bin` desde RisuAI existente y luego impórtelo en Kei-Risu. Funciona independientemente del entorno de origen (web / Tauri / Capacitor / local).

1. **En RisuAI existente**: Configuración > Copia de seguridad > "Guardar copia local" para exportar un archivo `.bin`.
2. **En Kei-Risu**: Configuración > Migración de datos > "Importar copia local de Risu original" para importar el archivo `.bin`.
...
2. En Kei-Risu, vaya a Configuración > Migración de datos > acordeón "Importar carpeta save de NodeOnly Risu".
...
1. Detenga el servidor Kei-Risu.
2. Sobrescriba la carpeta `save` de Kei-Risu con la carpeta `save` del RisuAI existente.
3. Reinicie el servidor Kei-Risu — la migración automática comienza.
