...
There are two ways to access Kei-Risu running on your PC from another device (smartphone, tablet, another PC).

- [1. Quick Tunnel](#1-quick-tunnel) — Built into Kei-Risu. No additional app required. URL changes on server restart.
- [2. Tailscale](#2-tailscale) — Based on a private network (VPN). Persistent URL.


## 1. Quick Tunnel

A built-in feature of Kei-Risu that issues a temporary remote access URL.

1. On your PC, in Kei-Risu: Settings > Remote Access > "Open Remote Access"
...
In the terminal on the PC running Kei-Risu, run once:
