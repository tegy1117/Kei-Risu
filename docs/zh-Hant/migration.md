...
從現有 RisuAI 安裝(Web RisuAI、本地 RisuAI)遷移資料到 Kei-Risu 有三種方式。根據您的來源環境和資料規模選擇。
...
從現有 RisuAI 匯出 `.bin` 備份檔案,然後匯入 Kei-Risu。無論來源環境(web / Tauri / Capacitor / 本地)如何均可使用。

1. **在現有 RisuAI 中**: 設定 > 備份 > "儲存本地備份" 匯出 `.bin` 檔案。
2. **在 Kei-Risu 中**: 設定 > 資料遷移 > "匯入原版 Risu 本地備份" 匯入 `.bin` 檔案。
...
2. 在 Kei-Risu 中開啟 設定 > 資料遷移 > "從 NodeOnly Risu 匯入 save 資料夾" 摺疊面板。
...
1. 停止 Kei-Risu 伺服器。
2. 用現有 RisuAI 的 `save` 資料夾整體覆蓋 Kei-Risu 的 `save` 資料夾。
3. 重啟 Kei-Risu 伺服器 — 自動遷移開始。
    - 可在終端機或 PM2 日誌中檢視進度。
