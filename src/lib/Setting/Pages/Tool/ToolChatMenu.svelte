<script lang="ts">
    import { CircleCheckIcon, XIcon } from '@lucide/svelte'
        ...
        <div class="w-full mt-4 flex flex-col border border-selected rounded-md">
            {#each DBState.db.tools.filter((tool) => !search || `${tool.name} ${tool.namespace}`.toLowerCase().includes(search.toLowerCase())) as tool, index}
                ...
                <div class="p-3 flex items-center gap-2">
                    <div class="grow"><div class:opacity-60={DBState.db.enabledTools.includes(tool.id)}>{tool.name}</div><div class="text-xs text-textcolor2">{tool.namespace}</div></div>
