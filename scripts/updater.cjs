...
function getCustomBackupKeepEntry() {
    ...
    try {
        ...
        if (!rel) {
            error('Custom backup directory points at the Kei-Risu app root. Move it to a separate folder before updating.');
        }
        const top = rel.split(path.sep)[0];
        if (MANAGED_BACKUP_PATH_ROOTS.has(top)) {
            error(`Custom backup directory is inside Kei-Risu app files (${rel}). Move it to a separate folder such as data/backups before updating.`);
        }
        return top || null;
    } catch {
        return null;
...
function httpsGet(url, redirectCount = 0) {
    return new Promise((resolve, reject) => {
        ...
        get(url, { headers: { 'User-Agent': 'Kei-Risu-Updater' } }, (res) => {
...
async function main() {
    ...
    for (const entry of fs.readdirSync(ROOT)) {
        ...
        try {
            ...
            error(isWin
                ? 'Update failed because some files are in use. Close the running Kei-Risu window/console first, then run update.bat again.'
    ...
    log('');
    if (isWin) {
        log('Restart by running Kei-Risu.exe');
    } else {
